         ...
         $show_no_features= esc_html ( get_option('wp_estate_show_no_features','') );
            ...
            if($show_no_features!='no'){
                ...
                foreach($feature_list_array as $checker => $value){
                    ...
                    if (esc_html( get_post_meta($post_id, $input_name, true) ) == 1) {
                        $return_string .=  '<div class="listing_detail col-md-6">' . trim($value) . '</div>';
                    }
                }
...
if( !function_exists('estate_listing_price') ):
    function estate_listing_price($post_id){
        ...
        if ($checkin_checkout_change_over!=0){
            $return_string.='<div class="listing_detail list_detail_prop_book_starts_end"><span class="item_head">'.esc_html__( 'Booking starts/ends only on','wpestate').':</span> ' .$week_days[$checkin_checkout_change_over] . '</div>'; 
        }
...
if( !function_exists('estate_listing_address') ):
    function estate_listing_address($post_id){
        ...
        if ($property_address != ''){
            $return_string.='<div class="listing_detail list_detail_prop_address"><span class="item_head">'.esc_html__( 'Address','wpestate').':</span> ' . $property_address . '</div>'; 
        }
        if ($property_city != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_city"><span class="item_head">'.esc_html__( 'City','wpestate').':</span> ' .$property_city. '</div>';  
        }  
        if ($property_area != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_area"><span class="item_head">'.esc_html__( 'Area','wpestate').':</span> ' .$property_area. '</div>';
        }    
        if ($property_county != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_county"><span class="item_head">'.esc_html__( 'County','wpestate').':</span> ' . $property_county . '</div>'; 
        }
        if ($property_state != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_state"><span class="item_head">'.esc_html__( 'State','wpestate').':</span> ' . $property_state . '</div>'; 
        }
        if ($property_zip != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_zip"><span class="item_head">'.esc_html__( 'Postcode','wpestate').':</span> ' . $property_zip . '</div>';
        }  
        if ($property_country != '') {
            $return_string.= '<div class="listing_detail list_detail_prop_contry"><span class="item_head">'.esc_html__( 'Country','wpestate').':</span> ' . $property_country . '</div>'; 
        } 
...
if( !function_exists('estate_listing_details') ):
    function estate_listing_details($post_id){
        ...
        if ($property_status != '' && $property_status != 'normal' ){
            $return_string.= '<div class="listing_detail list_detail_prop_status"><span class="item_head">'.esc_html__( 'Property Status','wpestate').':</span> ' . $property_status . '</div>';
        }   
        
        $return_string.= '<div class="listing_detail list_detail_prop_id"><span class="item_head">'.esc_html__( 'Property ID','wpestate').':</span> ' . $post_id . '</div>';
      
        
        if ($property_size != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_size"><span class="item_head">'.esc_html__( 'Property Size','wpestate').':</span> ' . $property_size . '</div>';
        }               
        if ($property_lot_size != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_lot_size"><span class="item_head">'.esc_html__( 'Property Lot Size','wpestate').':</span> ' . $property_lot_size . '</div>';
        }      
        if ($property_rooms != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_rooms"><span class="item_head">'.esc_html__( 'Rooms','wpestate').':</span> ' . $property_rooms . '</div>'; 
        }      
        if ($property_bedrooms != ''){
            $return_string.= '<div class="listing_detail list_detail_prop_bedrooms"><span class="item_head">'.esc_html__( 'Bedrooms','wpestate').':</span> ' . $property_bedrooms . '</div>'; 
        }     
        if ($property_bathrooms != '')    {
            $return_string.= '<div class="listing_detail list_detail_prop_bathrooms"><span class="item_head">'.esc_html__( 'Bathrooms','wpestate').':</span> ' . $property_bathrooms . '</div>'; 
        }      
        ...
        if( !empty($custom_fields)){  
           ...
           //    $slug =   sanitize_key ( str_replace(' ','_',$name) );
               ...
               if($value!=''){
                   $return_string.= '<div class="listing_detail list_detail_prop_'.( strtolower( str_replace(' ','_',$label) ) ).' "><span class="item_head">'.ucwords($label).':</span> ' .stripslashes($value). '</div>'; 
